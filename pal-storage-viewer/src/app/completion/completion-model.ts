...
export function summarize(record: PlayerCompletion, data: CompletionData, world?: WorldProgress): CompletionSummary {
  ...
  const stats = [
    ...
    stat('Species Captured', counters.tribe_captures, 'Distinct species captured'),
    stat('Total Pals Captured', countTotal(record.capture_counts), 'Lifetime captures, including repeats'),
    stat('Fishing Catches', countTotal(record.fishing_counts), 'Lifetime fishing catches across all species and sizes'),
    stat('Butchered', countTotal(record.butcher_counts), 'Total butchering counts'),
    stat('Awakenings', counters.awakenings, 'Total awakenings'),
    stat('Mutations', counters.mutations, 'Mutated pals bred'),
    condensationStat(record),
    stat('Items Crafted', countTotal(record.crafted_item_counts), 'Lifetime items crafted'),
    ...
    stat('Towers', countTotal(record.tower_boss_counts), 'Total tower boss victories, including repeat clears and hard mode'),
