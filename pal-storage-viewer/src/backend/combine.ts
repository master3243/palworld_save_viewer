...
import type { PlayerAttributes } from './player-progress';
...
function applyParsedFile(parsed: ParsedFile, source: SaveSource, set: SaveSet): void {
  ...
  if (parsed.kind === 'dimensional_storage') {
    ...
    source.note = 'Encountered species for the local player';
  ...
  const getSet = (label: string, letter: string): SaveSet => {
    ...
    if (!set) {
      set = {
        ...
        player_attributes: new Map(), key_item_containers: new Map(), item_counts: {}, seen_species: null, local_files: 0,
      };
      sets.set(label, set);
    }
...
export function flattenRecord(item: PalRecord): Row {
  ...
  return {
    ...
    work_species: item.work_species_ranks.join(','),
    ...
    stat_parts: item.derived.stat_parts,
