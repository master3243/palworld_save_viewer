...
Usage: python3 db/build_completion_data.py
...
HERE = ROOT / "resources" / "completion"
...
def map_data() -> dict:
    ...
    for name in ("palpagos", "tree"):
        ...
        for marker in grab("fixedDungeon"):
            ...
            elif marker.get("type") == "Tower" and marker.get("boss"):
                flag = "BOSS_BATTLE_NAME_" + marker["boss"].split("::")[-1]
                result["towers"][flag] = [round(pos[axis]) for axis in ("X", "Y", "Z")]
...
def build(cache: Path) -> dict:
    ...
    for tid, (ft_id, name) in EXTRA_TOWERS.items():
        ...
        towers[tid] = [name, *(coords(point) if point else paldb["towers"].get(tid, [0, 0, 0]))]
...
def main() -> None:
    from build_tracker_maps import build as build_maps
    cache = RAW
